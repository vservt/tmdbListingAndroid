...
@Composable
fun MovieDetailsScreen(id: Int, repository: TmdbRepository, navController: NavHostController) {
    val detailState = remember { mutableStateOf<com.example.tmdb.data.model.MovieDetail?>(null) }
    ...
    LaunchedEffect(id) {
        detailState.value = repository.movieDetail(id)
    }
    ...
    Column(
        ...
        Row(horizontalArrangement = Arrangement.SpaceEvenly, modifier = Modifier.fillMaxSize().padding(top = 8.dp)) {
            AsyncImage(
            ...
            Column(modifier = Modifier.padding(start = 12.dp).weight(1f)) {
                ...
                Text("Year: ${(detail.releaseDate ?: "").take(4)}", color = Color.White)
                Spacer(Modifier.height(12.dp))
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(onClick = {
                        scope.launch { repository.playStream(detail.id, MediaType.MOVIE) }
                    }) { Text("Watch") }
